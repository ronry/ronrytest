 ...
 * Vector, and run, which is requred be Runnable (this is a threaded class) and
 * grabs any items off the Vector. It uses a combination of wait() and notify()
 * to keep the run method from polling on an empty queue.
 * 
...
public class MessageQueue implements Runnable {
	...
	private LinkedList<SocketData> queue;
	...
	public MessageQueue(SocketConnection w) {
		writer = w;
		queue = new LinkedList<SocketData>();
		running = true;
		empty = true;
		new Thread(this).start();
	}

	synchronized public boolean enqueue(SocketData obj) {
		queue.addLast(obj);
		if (empty) {
			empty = false;
			notify();
