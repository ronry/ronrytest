...
/* -------------------- JavaDoc Information ----------------------*/
/**
 * A threaded connection class to maintain the conneciton to the client a new
 * ClientConnection must be create for each new client
 * 
...
public class ClientConnection implements Runnable, SocketConnection {
	 ...
	 * 
	 * @param sd
	 *            A SocketData object to be sent
	 */
	public void writeObject(SocketData obj) {
		queue.enqueue(obj);
	}
	...
	/**
	 * used to finalize the conneciton to the client. when a user connects he
	 * must send a SD_UserAdd to tell the server the requested name
	 * 
	 * @param username
	...
	/**
	 * used to rename the client <i>ADD USERNAME VALIDITY CHECK</a>
	 * 
	 * @param on
	 *            the old name of the client
	 * @param nn
	 *            the new (requested) name of the client
	 */
	...
	/**
	 * used to kick a user; eventually IP banning will be an added option
	 * (called ban), but will use SD_Kick
	 * 
	 * @param username
	 *            the name of the user to kicko
	 */
	public void kick(String username) {
		if (admin) {
			ClientConnection cc = (ClientConnection) server.connectedUsers
					.get(username);
			if (cc == null) {
				writeObject(new SD_Error("user " + username + " does not exist"));
			} else {
	 ...
	 * 
	 * @param nc
	 *            true for a new channel, if the channel exists an error is sent
	 *            back to the user, otherwise the channel is created and a
	 *            recursive call is made to switch the user to the new channel;
	 *            if false it checks the validity of the request and move the
	 *            user
	 * @param n
	 *            the name of the channel
	 * @param p
	 *            password for the channel, if one
	 */
			...
			if (server.channels.channelExists(n)) {
				if (channel.equals(n)) {
					...
					if (server.channels.userDel(channel)) {
						server.broadcast(new SD_Channel(true, channel, null),
								null);
	 ...
	 * 
	 * @param username
	 *            the name of the user, not used in this case (it should be
	 *            null)
	 * @param message
	 *            the messsage sent by the user
	 */
	 ...
	 * 
	 * @param username
	 *            the name of the user that the whisper is to be sent
	 * @param message
	 *            the message that is to be whispered
	 */
	...
	/**
	 * Method to implement runnable, listens for incoming objects
	 */
	public void run() {
		SocketData sd;
