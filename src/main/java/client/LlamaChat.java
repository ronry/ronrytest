 ...
 * user, port is the port on which the server is running, site is the IP address
 * or hostname of the server, location is the location of the client applet on
 * the web
 * 
 * @author Joseph Monti <a
 *         href="mailto:countjoe@users.sourceforge.net">countjoe@users.
 *         sourceforge.net</a>
 * @version 0.8
 */
/*
 * For use in a webpage: <OBJECT
 * classid="clsid:8AD9C840-044E-11D1-B3E9-00805F499D93" width="615" height="360"
 * codebase=
 * "http://java.sun.com/products/plugin/autodl/jinstall-1_4-windows-i586.cab#Version=1,4,0,mn"
 * > <PARAM name="code" value="client/LlamaChat.class"> <PARAM name="archive"
 * value="LlamaChat.jar"> <PARAM name="type"
 * value="application/x-java-applet;version=1.4"> <PARAM name="scriptable"
 * value="true"> <param name="username" value="[replace with username]"> <param
 * name="port" value="[replace with port]"> <COMMENT> <EMBED
 * type="application/x-java-applet;version=1.4" width="615" height="331"
 * code="client/LlamaChat.class" archive="LlamaChat.jar"
 * pluginspage="http://java.sun.com/j2se/1.4.1/download.html"
 * username="[replace with username]" port="[replace with port]"> <NOEMBED> No
 * Java 1.4 plugin </NOEMBED></EMBED> </COMMENT> </OBJECT>
 */
public class LlamaChat extends JApplet {
	...
	public void init() {
		...
		site = url.getHost();
		locationURL = "http://" + site + ":" + url.getPort() + "/"
				+ url.getPath();
		...
		public void actionPerformed(ActionEvent ae) {
			...
			if (ae.getSource() == butChannel) {
				...
				String channel = JOptionPane.showInputDialog(
						...
						"Enter the name of the channel", "Create Channel",
				...
				String pass = JOptionPane.showInputDialog(
						(Component) ae.getSource(), message, "Join Channel",
						JOptionPane.QUESTION_MESSAGE);
	...
	/**
	 * a Subclass to handle cell items in the user list lets us modify add an
	 * icon to the username
	 */
	protected final class MyCellRenderer extends DefaultListCellRenderer {
		final ImageIcon normal = new ImageIcon(getURL("images/face.gif"));
		final ImageIcon ignored = new ImageIcon(
				getURL("images/face_ignore.gif"));
		final ImageIcon afk = new ImageIcon(getURL("images/afk.gif"));
		final ImageIcon admin = new ImageIcon(getURL("images/admin.gif"));
	 ...
	 * 
	 * @param filename
	 *            the name of the file to be turned into URL
	...
	/**
	 * Method called when a user message has been sent. checks to see if it is a
	 * command or regular message
	 */
	...
	public boolean parseCommand(String cmd) {
		...
		if (cmd == "help" || cmd == "?") {
			...
			return true;
			/*
			 * } else if (cmd.startsWith("proxy")) { int start =
			 * cmd.indexOf(' '); if (start < 0) {
			 * error("usage: \\proxy &lt;host&gt; &lt;port&gt;"); return false;
			 * } String phost = cmd.substring(start+1); start =
			 * phost.indexOf(' '); if (start < 0) {
			 * error("usage: \\proxy &lt;host&gt; &lt;port&gt;"); return false;
			 * } String pport = phost.substring(start+1); phost =
			 * phost.substring(0, start); Properties systemSettings =
			 * System.getProperties(); systemSettings.put("proxySet", "true");
			 * systemSettings.put("proxyHost", phost);
			 * systemSettings.put("proxyPort", pport);
			 * System.setProperties(systemSettings); serverMessage("Using " +
			 * phost + ":" + pport +
			 * " as proxy server<br>you can type \\reconnect to reconnect with this proxy"
			 * ); return true;
			 */
	 ...
	 *            the user to ignore
	 * @param quite
	 *            if true will not show confirmation
	...
	/**
	 * Sends a text to the chat window. Parses the message to pick out
	 * emoticons.
	 * 
	 * @param un
	 ...
	 * @param whisper
	 *            indicates the message was a wisper and makes the message
	 *            italic
	 */
	...
	/**
	 * sorts the user list and rebuilds the user list from the sorted user
	 * vector,
	 */
	...
	/**
	 * changes the name of a user, updating list of admins, afks, ignoes, and
	 * master user list
	 * 
	 * @param on
	 *            old username
	 ...
	 * 
	 * @param un
	 *            the name of the user sending the chat
	 * @param message
	 ...
	 * 
	 * @param un
	 *            the name of the user sending the whisper
	 * @param message
	 *            the message that was sent
	 */
	...
	/**
	 * used to manage the connection icons to signify connection and secure
	 * status
	 * 
	 * @param b
	 *            true if connected
	...
	/**
	 * creates a new channel, if the channel exists it is removed (this method
	 * doubles as a removeChannel)
	 * 
	 * @param name
